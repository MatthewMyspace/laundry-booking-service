...
const TaskForm = ({ Booking, setBooking, editingTask, setEditingTask }) => {
  const { user } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingTask) {
        const response = await axiosInstance.put(`/api/Booking/${editingTask._id}`, formData, {
          headers: { Authorization: `Bearer ${user.token}` },
        });
        setBooking(Booking.map((task) => (task._id === response.data._id ? response.data : task)));
      } else {
